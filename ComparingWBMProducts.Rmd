...
Next, we load in model parameters associated with the 10 randomly-selected points. These parameters are inputs to the WBM and were supplied within the AWS directory. The directory also includes the R script that was presumably used to generate these parameters (aws_r_tutorial/get_site_params.R). However, the specific datasets (rasters) that the get_site_params.R script loads are not available to us. Therefore, we instead rely on the previously compiled .csv file that is available.
...
ggplotly(
ggplot() +
    geom_point(data = WBData_online_centroid, aes(x = as_date(Date), y = `runoff in`, color = "Centroid"), size = 0.75) +
    geom_point(data = grid_runoff, aes(x = date, y = mean_runoff, color = "Gridded"), alpha = 0.6, size = 0.75) +
    geom_line(data = WBData_coded, aes(x = as_date(Date), y = mean_runoff, color = "R Code"), cex = 0.2, alpha = 0.8) +
    #geom_point(data = WBData_coded, aes(x = as_date(Date), y = mean_runoff, color = "R Code"), size = 0.75) +
    theme_bw() +
...
#ggplotly(
  ggplot() +
    geom_point(data = WBData_online_centroid, aes(x = as_date(Date), y = `PET in`, color = "Centroid"), size = 0.75) +
    geom_point(data = grid_pet, aes(x = date, y = mean_pet, color = "Gridded"), alpha = 0.6, size = 0.75) +
...
The above plots show differences in runoff, PET, and rain from the three NPS WBM products. There is no clear bias between models, i.e., over- or under-estimation between two methods. The fact that rain differs between the two models suggests that differences are occurring early-on in the model, likely with rain-snow partitioning.

Based on discussions with the NPS group, we understand that published centroid WBM products and gridded WBM products were developed with different intentions and are inherently different by design. Key differences that we have identified by comparing the two code sources (M. Tercyk's python script (aws_python_script) and A. Runyon's R script (above) include:
...
-   Other discrepancies
    ...
    -   In the R WBM code, there is a potential redundancy in calculating PET. The `get_OudinPET()` function duplicates parts of the `modify_PET()` function. This could be intentional if the modify_PET function is not used with the Oudin method of calculating PET. However, both Oudin and modify_PET are used in the published tutorial on AWS. (See lines 107-120 above.) Below are the two functions:
...
ggplot() +
  geom_point(data = WBData_coded, aes(x = as_date(Date), y = mean_runoff, color = "With ET Mod"), cex = 1.5) +
  geom_line(data = WBData_coded_no_modify_PET, aes(x = as_date(Date), y = mean_runoff, color = "Without ET Mod"), alpha = 0.5) +
  #geom_col(aes(x = as_date(WBData_coded$Date), y = WBData_coded$mean_runoff - WBData_coded_no_modify_PET$mean_runoff), color = "#578010") +
  theme_bw() +
  scale_color_manual("", values = c("#578010","black")) +
